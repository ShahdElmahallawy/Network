...
When started, it prompts you to enter a name, and the entered name is registered on the server.
...
int main(){
	...
	int chkerr;		//chkerr:A variable that holds the return value of the system call
	...
	if(sockfd < 0) myerror("socket_error");		//Error check, call myerror on error
